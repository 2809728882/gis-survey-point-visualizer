...
面向基建施工现场的轻量级测绘点位管理平台，提供二维地图、施工测点标注、坐标定位、范围圈定、影像叠加、数据导入转换和施工进度状态展示。

## 项目定位

本项目适合施工测量、控制点布设、现场放样辅助、施工进度核查和安全隐患排查等场景。前端 Demo 使用 OpenLayers 构建，可直接在浏览器中运行；数据处理脚本使用 Python 将 GNSS/RTK、全站仪、CSV/Excel 等外业数据整理为前端可加载的 JSON。
...
```text
OpenLayers-based construction survey point visualization demo with Python data conversion tools.
```
